...
- **대화방 (`/finz/party/[groupId]`):** `finz-party-room`(오케스트레이터·폴링·낙관적 전송·nudge·`@finz` 인텐트 라우팅·차트·선제 개입·정기메시지 등록), `finz-chat-header`(헤더에 ⚙️ 설정 버튼), `finz-chat-timeline`(카카오톡식 날짜 구분선), `finz-chat-message-view`(kind별, 메시지별 시각 `오전/오후 h:mm`), `finz-chat-composer`(+메뉴 "대화 요약"), `finz-chart-bubble`(TradingView 위젯), `finz-nudge-bubble`, `finz-position-input`, `finz-room-join-view`(원탭 합류), `finz-invite-sheet`(친구 초대/링크), `finz-room-full-notice`.
...
- **API:** `account`(+`/handle`), `friends`, `feed`, `rooms`(+`/[groupId]/invite`·`/join`·`/self`), `party/[groupId]/{message,chat,ask,intent,proactive,pick,pick/summary,summary,position,chart,briefing/subscribe,recurring,recurring/parse,recurring/[id],portfolio,portfolio/handle,portfolio/[tradeId],join}`, `cron/{briefing,recurring}`(스케줄러 전용, Bearer `CRON_SECRET`).
