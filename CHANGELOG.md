...
### Added
- **FINZ 현재 채팅방 월간 리뷰.** `@finz 월간리뷰해줘`를 부르면 현재 방의 저장된 대화를 기준으로 주요 미국 주식/코인 언급, 사용자별 방향, 실제 가격 움직임을 같은 채팅방에 FINZ 메시지로 올린다.
- **정기 월간 리뷰 cron.** Vercel cron이 매일 KST 12:00에 호출되고, 서버가 한국시간 말일일 때만 방별 정기 리뷰를 생성한다. 중간 리뷰는 정기 리뷰 기준선을 바꾸지 않는다.

### Internal
- 월간 리뷰 엔진(`finz-monthly-review.ts`)과 Yahoo Finance 기반 가격 provider를 추가했다. 최초 리뷰는 첫 확보 가능 시가를 기준으로 삼고, 이후 정기 리뷰는 전달 정기 리뷰 시가를 기준으로 등락률을 계산한다.
- FINZ 방 전역 인덱스를 추가해 cron이 활성 방을 순회할 수 있게 했다.
