...
MVP 1의 핵심 흐름은 다음 5단계다.

1. 취향 카드 선택
2. 개인 투자 캐릭터 소환
3. 친구 그룹 생성 또는 초대
4. AI가 그룹 조합 기반으로 오늘의 우정주 제안
5. 투자 레이드로 한 줄 포지션과 대화 시작
...
AI가 그룹의 캐릭터 조합과 카드 태그를 바탕으로 오늘 이야기하면 재밌을 종목 하나를 제안한다.
...
- AI 실패 시 사전 정의된 예시 종목/테마 fallback으로 레이드까지 이어지게 한다.
