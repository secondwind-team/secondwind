...
| `portfolio` | finz 봇 | 왼쪽 + 🤖 아바타 + **포트폴리오 카드**(`finz-portfolio-card`) — 보유 종목·평단·원가·평가손익/수익률(현재가 그라운딩)·비중 바·섹터 분석(view:sector)·청산 실현손익. 누구의/공동 여부 헤더. 보유 현황은 거래(append-only)에서 결정적 계산, 현재가만 그라운딩, "참고용" 면책+출처. 손익색 `--fz-gain/--fz-loss` |
...
- **월간 투자 대화 리뷰:** 메시지가 쌓인 방을 `sw:finz:monthly-review:rooms` SET 에 등록하고, `@finz 월간리뷰해줘` 또는 `+` 메뉴에서 중간 리뷰를 만든다. 정기 리뷰는 GitHub Actions가 매일 12:00 KST에 보안 엔드포인트를 호출하되 서버가 KST 말일만 통과시킨다. 사용자별 종목·방향은 결정적으로 추출하고 Yahoo 일봉 시가와 비교한다. 중간 리뷰는 전달 정기 리뷰의 가격 기준선을 바꾸지 않으며, 방·월 단위 락으로 중복 생성을 막는다.
...
- **대화방 (`/finz/party/[groupId]`):** `finz-party-room`(오케스트레이터·폴링·낙관적 전송·nudge·`@finz` 인텐트 라우팅·차트·선제 개입·정기메시지 등록), `finz-chat-header`(헤더에 ⚙️ 설정 버튼), `finz-chat-timeline`(카카오톡식 날짜 구분선), `finz-chat-message-view`(kind별, 메시지별 시각 `오전/오후 h:mm`), `finz-chat-composer`(+메뉴 "대화 요약"·"월간 투자 리뷰"), `finz-chart-bubble`(TradingView 위젯), `finz-nudge-bubble`, `finz-position-input`, `finz-room-join-view`(원탭 합류), `finz-invite-sheet`(친구 초대/링크), `finz-room-full-notice`.
- **채팅방 설정 (`/finz/party/[groupId]/settings`, 탭바 밖 풀블리드):** `finz-room-settings` — 정기 메시지 목록·토글·수정·삭제·추가폼 + 내장 아침 시황 토글 + **포트폴리오**(`finz-portfolio-settings` — 개인/공동 전환·보유 요약·거래내역 추가/수정/삭제) + 향후 기능(사진/유형별 보기 등) 로드맵. members-only(클라 게이트 + 서버 members-guard).
- **포트폴리오:** 순수 모델 `finz-portfolio.ts`(거래→보유 결정적 계산: 평균단가법·실현손익·통화별 요약·비중·섹터, 단위 테스트), 저장 `finz-portfolio-store.ts`(거래 append-only LIST), 카드 `finz-portfolio-card`. 현재가는 그라운딩 LLM, 섹터는 LLM 분류. 손익 의미색 `--fz-gain #2f9e6b`/`--fz-loss #d8503f`(finz 네임스페이스).
- **시각·날짜:** 순수 헬퍼 `finz-time.ts`(KST 고정 `formatKstTime`/`formatKstDate`/`kstDayKey`).
- **대화 요약:** 순수 헬퍼 `finz-summary.ts`(`resolveSummaryWindow` — 명시 기간/기본 100개 규칙) + 라우트 `summary`. 우정주 입장 기반 **파티 요약**(`pick/summary` + `finz-party-summary` 앰버 카드)과 별개로 공존.
- **정기 메시지:** 순수 모델 `finz-recurring.ts`(스케줄 계산·라벨·검증, KST), 저장 `finz-recurring-store.ts`(정의 blob + 방별 SET + 전역 due ZSET + 발송 락), UI `finz-room-settings`.
- **재사용:** `finz-character-card`(헤더·프로필·조인 아바타), `finz-party-pick-result`(픽 말풍선), `finz-party-summary`(파티 요약 앰버 말풍선), `finz-chart-bubble`(TradingView 차트 말풍선), `finz-character-builder`(프로필의 캐릭터 소환).
- **API:** `account`(+`/handle`), `friends`, `feed`, `rooms`(+`/[groupId]/invite`·`/join`·`/self`), `party/[groupId]/{message,chat,ask,intent,proactive,pick,pick/summary,summary,monthly-review,position,chart,briefing/subscribe,recurring,recurring/parse,recurring/[id],portfolio,portfolio/handle,portfolio/[tradeId],join}`, `cron/{briefing,recurring,monthly-review}`(스케줄러 전용, Bearer `CRON_SECRET`).
...
| 날짜 | 결정 | 근거 |
|---|---|---|
| 2026-07-16 | **대화방 월간 투자 리뷰 + 모바일 입력 확대 방지** | 사용자 요청. 대화의 미국 주식·주요 코인 언급과 사용자별 방향을 결정적으로 추출해 Yahoo 일봉 시가 변동으로 회고한다. 중간 리뷰는 `@finz` 명령·`+` 메뉴, 정기 리뷰는 KST 말일 GitHub Actions로 제공하며 전달 정기 리뷰만 다음 가격 기준선을 갱신한다. iOS Safari의 16px 미만 입력 포커스 자동 확대는 모바일 `.fz-input`을 16px로 맞추고 데스크톱은 기존 14px을 유지해 해결한다. |
...
| 2026-06-28 | **대화방 포트폴리오(매매기록·평단·실현/평가손익·섹터·차트)** | 사용자 요청. 거래(매수/매도)를 append-only 이벤트로 기록하고 보유 현황(평단·수량·실현손익)은 결정적 순수 계산(평균단가법) — 채팅 타임라인 철학 그대로(환각 0). 현재가/평가손익만 그라운딩 LLM(`SYMBOL=가격` 관용 파싱, 출처·면책), 섹터는 LLM 분류, 비중은 자체 바 차트(외부 라이브러리 0), 종목 주가는 기존 TradingView. 누가/개인 vs 공동(scope) 태그. `@finz` 자연어(intent `portfolio`)로 기록·조회·섹터, 설정에서 거래 CRUD. 새 kind `portfolio` + 카드, 손익색 `--fz-gain/--fz-loss`(finz 네임스페이스 — emerald 금지 원칙 유지). 모든 변경 라우트 members-guard·인젝션 가드. |
| 2026-06-28 | **메시지 시각·날짜 + 대화 요약 복구 + 채팅방 설정 + 정기 메시지** | 사용자 요청 4종. ① 카카오톡식 날짜 구분선 + 메시지별 `오전/오후 h:mm`(KST 고정 순수 헬퍼, 서버 무변경 렌더링만). ② "요약" 인텐트가 우정주 파티 요약으로만 가 평범한 방에선 안 되던 걸 **전제조건 없는 일반 대화 요약**으로 복구(명시 기간/기본 100개, `resolveSummaryWindow` 결정적 파싱). 파티 요약은 nudge 흐름에 공존. ③ 채팅방 **설정 화면** 신설(헤더 ⚙️) — 확장 IA(사진/유형별 보기 등 로드맵 포함). ④ **정기 메시지** — `@finz` 자연어 등록(intent `schedule`, LLM 추출) 또는 설정 폼, 고정문구/AI생성 두 종류, 설정에서 관리. 발송은 기존 브리핑과 같은 GitHub Actions cron 패턴을 ~10분 간격으로 일반화(전역 due ZSET, at-most-once, 동일 `CRON_SECRET`). 모든 변경 라우트 members-guard·인젝션 가드 유지. |
