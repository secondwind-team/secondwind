...
| `FINZ-AI-04` | 현재 채팅방 월간 리뷰 | `live` | `@finz 월간리뷰해줘` → 현재 방의 저장된 대화에서 주요 미국 주식/코인 언급과 사용자별 방향을 뽑고, 전달 정기 리뷰 시가 대비 이번 리뷰 시가의 차이·등락률을 같은 채팅방에 올린다. 중간 리뷰는 정기 기준선을 바꾸지 않는다. | `app/api/finz/party/[groupId]/monthly-review`, `app/api/finz/cron/monthly-review`, `lib/common/services/finz-monthly-review.ts`, `lib/server/finz-monthly-review-store.ts`, `lib/server/finz-price-provider.ts` | 한국 주식/더 넓은 종목명 매칭, LLM 추출 고도화 | Yahoo Finance 장애/휴장 fallback, cron 대상 방 인덱스 self-heal |
