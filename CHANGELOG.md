...
### Fixed
- **푸시 알림 상태바 아이콘이 흰 네모로 뜨던 문제.** 안드로이드 상태바 알림 아이콘은 알파(실루엣)만 쓰고 색을 버리는데, `badge` 에 코랄이 꽉 찬 불투명 PNG(`icon-192.png`)를 줘서 전체가 흰 사각형으로 렌더됐다. 투명 배경 + 흰 "f" 모노크롬 아이콘(`badge-96.png`)을 새로 만들어 `badge` 로 지정 → 상태바에 f 실루엣으로 뜬다. 알림 카드의 큰 아이콘(`icon`)은 컬러 그대로.
  - 참고(코드 아님): heads-up(화면 상단 팝업 배너)은 안드로이드 알림 채널 중요도가 "높음/긴급"이어야 뜬다. 웹 알림 채널 중요도는 Chrome 이 관리해 앱 코드로 못 올린다 — 안 뜨면 기기 알림 설정에서 해당 사이트/Chrome 알림을 "긴급(팝업 표시)"으로, 방해금지 해제. (vibrate 옵션은 Android 8+ 에서 deprecated.)
