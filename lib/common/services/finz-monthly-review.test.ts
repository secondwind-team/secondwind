...
describe("월간 리뷰 명령과 일정", () => {
  it("@finz 월간리뷰 명령의 띄어쓰기 변형을 인식", () => {
    expect(isFinzMonthlyReviewCommand("@finz 월간리뷰해줘")).toBe(true);
    expect(isFinzMonthlyReviewCommand("@핀즈 월간 리뷰 해 줘")).toBe(true);
    expect(isFinzMonthlyReviewCommand("이번 달 투자 얘기 어땠어?")).toBe(false);
  });

  it("한국시간 말일만 정기 리뷰 대상으로 판단", () => {
    expect(isKstMonthEnd(new Date("2026-07-31T03:00:00.000Z"))).toBe(true);
    expect(isKstMonthEnd(new Date("2026-07-30T03:00:00.000Z"))).toBe(false);
  });

  it("같은 한국시간 월의 정기 리뷰 중복을 감지", () => {
    const previous = makePreviousScheduledReview();
    expect(hasScheduledReviewForKstMonth([previous], new Date("2026-06-30T14:00:00.000Z"))).toBe(true);
    expect(hasScheduledReviewForKstMonth([previous], new Date("2026-07-01T03:00:00.000Z"))).toBe(false);
  });
});
