...
- enrich 결과와 통계를 노출하고, 실패한 장소는 최대 6개까지 repair pass 로 재검증한다
- 속도와 안정성 사이의 기본 선택지
...
- 후보 검색 → 후보 중 선택 → 일정 구성의 2-pass 구조로 후보 밖 장소명 창작을 줄인다
...
1. `npm run eval:travel` 로 골든셋 반복 실행과 snapshot 저장을 시작한다.
2. eval 결과를 바탕으로 `verified` 의 후보 수, 보수성, 빈 `place_query` 허용 기준을 조정한다.
3. Google Search grounding, Kakao Place API, Google Places API 는 비용과 ToS 를 검토한 뒤 별도 실험 모델로 추가한다.
