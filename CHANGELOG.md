...
### Added
- **FINZ 월간 투자 대화 리뷰.** 대화방에서 나온 미국 주식·주요 코인 언급과 사용자별 방향을 모아 Yahoo 일봉 시가 변동과 함께 회고한다. `@finz 월간리뷰해줘` 또는 하단 `+` 메뉴로 중간 리뷰를 만들 수 있고, 대화가 있는 방은 KST 말일 12:00에 자동 정기 리뷰를 받는다. 중간 리뷰는 전달 정기 리뷰의 가격 기준선을 바꾸지 않으며 방·월 단위 락으로 중복 생성을 막는다.

### Fixed
- **모바일 채팅 입력 시 화면 자동 확대 방지.** iOS Safari가 16px 미만 입력창을 포커스할 때 확대하던 동작을 막기 위해 모바일 `.fz-input`을 16px로 조정하고, 640px 이상 화면은 기존 14px을 유지한다.

## [0.1.33.3] - 2026-07-05

### Added
- **채팅방별 사진 업로드 화질 설정.** 채팅방 설정(⚙️)에 "🖼 사진 화질"이 생겨 방 단위로 **원본 / 표준 / 저용량** 중 고를 수 있다(기본 **표준**). 원본이 실제로 가장 고화질이라 중간값을 "고품질"이라 부르면 오해될 수 있어 **표준**으로 명명했다 — 표준=긴 변 1600px(권장), 저용량=긴 변 1024px(데이터 절약), 원본=리사이즈 없음. 업로드 전 브라우저에서 리사이즈·JPEG 재인코딩하며(HEIC 은 항상 JPEG), 설정은 전 멤버에게 폴링으로 전파된다. 표준/저용량이면 큰 원본도 리사이즈로 12MB 안에 들어온다.

## [0.1.33.2] - 2026-07-05

### Added
- **HEIC 사진 지원 + 텍스트박스 이미지 붙여넣기.** 아이폰 기본 포맷인 HEIC/HEIF 을 첨부하면 업로드 시 자동으로 **JPEG 로 변환**해 저장한다 — 크롬·안드로이드 포함 어디서나 보인다(변환 라이브러리 `heic2any` 는 HEIC 을 고를 때만 lazy-load 해서 평소 채팅 번들엔 영향 없음). 또 이미지를 복사해 입력창에 **바로 붙여넣기(Cmd/Ctrl+V)** 하면 첨부로 추가된다.
