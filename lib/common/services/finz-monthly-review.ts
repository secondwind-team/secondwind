...
export function isFinzMonthlyReviewCommand(text: string): boolean {
  return /(?:^|\s)(?:@\s*(?:finz|핀즈|ai)\s*)?월간\s*리뷰(?:\s*해\s*줘)?(?:\s|$)/i.test(text);
}

export function isKstMonthEnd(date: Date): boolean {
...
export function hasScheduledReviewForKstMonth(
  reviews: Pick<FinzReviewRecord, "kind" | "createdAt" | "updatesMonthlyBaseline">[],
  date: Date,
): boolean {
  const target = kstYearMonth(date);
  return reviews.some((review) => {
    if (review.kind !== "scheduled-monthly" || !review.updatesMonthlyBaseline) return false;
    const createdAt = new Date(review.createdAt);
    return !Number.isNaN(createdAt.getTime()) && kstYearMonth(createdAt) === target;
  });
}
