...
- **채팅방 설정 (`/finz/party/[groupId]/settings`, 탭바 밖 풀블리드):** `finz-room-settings` — 정기 메시지 목록·토글·수정·삭제·추가폼 + 내장 아침 시황 토글 + 향후 기능(사진/유형별 보기 등) 로드맵. members-only(클라 게이트 + 서버 members-guard).
...
- **API:** `account`(+`/handle`), `friends`, `feed`, `rooms`(+`/[groupId]/invite`·`/join`·`/self`), `party/[groupId]/{message,chat,ask,intent,proactive,pick,pick/summary,summary,position,chart,briefing/subscribe,recurring,recurring/parse,recurring/[id],join}`, `cron/{briefing,recurring}`(스케줄러 전용, Bearer `CRON_SECRET`).
