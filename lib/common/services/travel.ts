...
// 사용자가 LLM 에 강제 주입하고 싶은 "반드시 가야 할 곳". stay 와 별개의 슬롯.
// PR 0 단계에선 textarea 기반이라 .place 는 항상 undefined. 추후 모달 (PR 5) 로 교체 시 채워짐.
...
export const MUST_VISIT_MAX_ITEMS = 10;
export const MUST_VISIT_NAME_MAX = 80;
export const MUST_VISIT_TEXTAREA_MAX = MUST_VISIT_MAX_ITEMS * (MUST_VISIT_NAME_MAX + 1);
...
export type TravelPlan = {
  ...
  caveats: string[];
  /** 사용자가 mustVisit 으로 지정했지만 LLM 이 일정에 포함하지 않은 장소들. UI 에서 명시 노출. */
  mustVisitMissing?: string[];
};
...
export function validateTravelInput(raw: unknown): TravelInputValidationResult {
  ...
  const mustVisit = parseMustVisitInput(r.mustVisit);
  ...
  if (mustVisit && mustVisit.length > 0) base.mustVisit = mustVisit;
...
// 한 mustVisit 안에서 같은 normalized name 중복은 제거. stay 와의 충돌은 허용 (PR 0 단계).
function parseMustVisitInput(raw: unknown): MustVisitItem[] | undefined {
  ...
  for (const item of raw) {
    ...
    const name = typeof r.name === "string" ? r.name.trim().slice(0, MUST_VISIT_NAME_MAX) : "";
    if (!name) continue;
    const key = normalizeMustVisitName(name);
    ...
    const place: PlaceInfo | undefined = placeRaw
      ...
      : undefined;
    const entry: MustVisitItem = { name };
...
// PR 0 의 sanitize 보호와 좌표 보존이 같은 normalize 규칙을 공유한다.
// fuzzy match 없음 — exact match only. (sanitize 는 hallucination 방어 역할이라
// 유사도 휴리스틱으로 통과시키면 다른 가게가 보호 받는 footgun.)
export function normalizeMustVisitName(s: string): string {
...
export type ParseTravelPlanOptions = {
  /** 사용자가 강제 주입한 mustVisit. sanitize 보호 + 좌표 보존 + 누락 검증의 입력. */
  mustVisit?: MustVisitItem[];
};

export function parseTravelPlan(raw: string, options: ParseTravelPlanOptions = {}): TravelPlan | null {
  ...
  const protectedNames = options.mustVisit?.map((item) => item.name) ?? [];
  sanitizeGeneratedPlan(parsed, { protectedNames });
  if (options.mustVisit && options.mustVisit.length > 0) {
    applyMustVisitToPlan(parsed, options.mustVisit);
  }
  return parsed;
}
...
type SanitizeOptions = {
  /** mustVisit 으로 보호된 이름 목록. exact-normalize 일치 시 suppression 면제. */
  protectedNames?: string[];
};

function sanitizeGeneratedPlan(plan: TravelPlan, options: SanitizeOptions = {}): void {
  const protectedKeys = new Set(
    (options.protectedNames ?? [])
      .map(normalizeMustVisitName)
  ...
  for (const day of plan.days) {
    day.items.forEach((item, index) => {
      ...
      if (isProtectedItem(item, protectedKeys)) return;
      if (shouldSuppressPlaceQuery(item)) {
        item.place_query = "";
      }
    });
  }
  suppressRepeatedPlaceQueries(plan, protectedKeys);
}

function isProtectedItem(item: TravelItem, protectedKeys: Set<string>): boolean {
  if (protectedKeys.size === 0) return false;
  const key = normalizeMustVisitName(item.place_query ?? "");
  return key.length > 0 && protectedKeys.has(key);
}
...
function suppressRepeatedPlaceQueries(plan: TravelPlan, protectedKeys: Set<string>): void {
  ...
  for (const day of plan.days) {
    for (const item of day.items) {
      ...
      if (seen.has(key)) {
        // mustVisit 으로 보호된 이름은 중복도 허용 (사용자가 같은 장소 재방문 의도).
        if (protectedKeys.has(normalizeMustVisitName(query))) continue;
        item.place_query = "";
        continue;
      }
      seen.add(key);
...
/**
 * mustVisit 후처리:
 * 1) mustVisit.place 가 있고 LLM 출력 place_query 와 exact-normalize 일치하는 day item 에 PlaceInfo 복사.
 *    enrich 단계 재호출 없이 사용자 좌표 그대로 반영.
 * 2) plan.mustVisitMissing 계산 — 일정에 포함되지 않은 mustVisit 이름 목록.
 *
 * fuzzy fallback 없음 — sanitize 와 같은 exact-normalize 정책. 사용자가 입력한
 * 이름과 LLM 출력의 미세 변형 (예: 후미 표기 추가) 은 의도적으로 매칭 안 함.
 * (LLM 이 SYSTEM_PROMPT 의 "장소명을 그대로 사용" 규칙을 따라야 보호받음.)
 */
export function applyMustVisitToPlan(plan: TravelPlan, mustVisit: ReadonlyArray<MustVisitItem>): void {
  if (mustVisit.length === 0) return;

  // mustVisit 이름 → MustVisitItem 인덱스 (normalized key 기준)
  const byKey = new Map<string, MustVisitItem>();
  for (const item of mustVisit) {
    const key = normalizeMustVisitName(item.name);
  ...
  for (const day of plan.days) {
    for (const dayItem of day.items) {
      const key = normalizeMustVisitName(dayItem.place_query ?? "");
      if (!key) continue;
      const mv = byKey.get(key);
      if (!mv) continue;
      matchedKeys.add(key);
      // mustVisit.place 가 있으면 좌표 복사. 없으면 enrich 가 자체 검색.
      if (mv.place && (typeof mv.place.lat === "number" || typeof mv.place.lng === "number")) {
        dayItem.place = { ...mv.place };
        dayItem.place_warning = undefined;
      }
    }
  }
  ...
  for (const item of mustVisit) {
    const key = normalizeMustVisitName(item.name);
    if (!key) continue;
    if (!matchedKeys.has(key)) missing.push(item.name);
  }
  if (missing.length > 0) {
    plan.mustVisitMissing = missing;
  }
}
...
export function isTravelPlan(v: unknown): v is TravelPlan {
  ...
  if (p.mustVisitMissing !== undefined && !isStringArray(p.mustVisitMissing)) return false;
  return true;
}
